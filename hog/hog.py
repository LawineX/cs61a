...
def more_boar(player_score, opponent_score):
    ...
    if get_left(player_score)<get_left(opponent_score) and get_next(player_score)<get_next(opponent_score):
        return True
    else:
        return False
    ...
    while flag:
        if score0<goal and score1<goal:
            if who==0:
                ...
                score0+=take_turn(num_roll,score1,dice,goal)
                if not more_boar(score0,score1):
                    who=next_player(who)
            elif who==1:
                num_roll=strategy1(score1,score0)
                score1+=take_turn(num_roll,score0,dice,goal)
    ...
    def say(score0,score1):
        if who==0:
            ...
            if temp>running_high:
                print("Player",who,"has reached a new maximum point gain.",temp,"point(s)!")
            score=score0
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    index=None
    for i in range(10):
        ret=make_averaged(roll_dice,trials_count)
        result=ret(i+1,dice)
        if result>max:
            max=result
            index=i+1
...
def more_boar_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    return 0 if more_boar(score+piggy_points(opponent_score),opponent_score) else 0 if piggy_points(opponent_score)>=cutoff else num_rolls  # Replace this statement
    # END PROBLEM 11

def best_cutoff(num_rolls):
    h=make_averaged(roll_dice)
    avg=h(num_rolls)
...
def final_strategy(score, opponent_score, num_rolls=6):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    cutoff=lambda num_rolls:make_averaged(roll_dice)(num_rolls)
    def fin():
        if more_boar(score+piggy_points(opponent_score),opponent_score):
            ...
            return 2
        elif score + roll_dice(3) >= 100:
            return 3
        elif piggy_points(opponent_score)>=cutoff(num_rolls):
            return 0
        else:
            return num_rolls
    return fin()  # Replace this statement
    # END PROBLEM 12
