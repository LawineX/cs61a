...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while current is not beehive:
            if current.bees and self.min_range<=num<=self.max_range:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.chewing:
            ...
            if bee_selector(self.place.bees):
                bee_selector(self.place.bees).reduce_health(bee_selector(self.place.bees).health)
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8
        return True if self.contained_ant is None and  not other.is_container() else False
        # END Problem 8
...
# END Problem EC
    ...
    def __init__(self,health=1):
        # BEGIN Problem EC
        super().__init__(health)
        self.num=QueenAnt.has_deployed
        QueenAnt.has_deployed+=1
        self.buff_list=[]
        # END Problem EC
    ...
    def action(self, gamestate):
        ...
        if self.num==0:
            next_sata=self.place.exit
            while next_sata:
                if next_sata.ant:
                    if next_sata.ant not in self.buff_list:
                        next_sata.ant.damage*=2
                        self.buff_list.append(next_sata.ant)
                    if next_sata.ant.is_container() and next_sata.ant.contained_ant and next_sata.ant.contained_ant not in self.buff_list:
                        next_sata.ant.contained_ant.damage*=2
                        self.buff_list.append(next_sata.ant.contained_ant)
                next_sata=next_sata.exit
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        return self.place.ant.blocks_path if self.place.ant else False
    ...
    def slow(self, length):
        ...
        self.action=self.apply_status(slow_status,Bee.action,length)
    ...
    def scare(self, length):
        ...
        if not self.flag:
            self.action=self.apply_status(scare_status,Bee.action,length)
            self.flag=1
        # END Problem Optional 2
    ...
    def apply_status(self, status, previous_action, length):
        ...
        def inner(gamestate):
            ...
            if self.length is False:
            ...
            elif self.length==3 and length==3:
                self.length-=1
                return status(gamestate)
            elif self.length==2 and length==3:
                self.length-=1
                length-=1
                return status(gamestate)
            elif self.length==1 and length==2:
                self.length-=1
                return status(gamestate)
            elif length>0:
                length-=1
                ...
                return previous_action(self,gamestate)
        return inner
        # END Problem Optional 2
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        dit={}
        while current is not beehive:
            if current.ant is not self and current.ant is not None:
                dit[current.ant]=num
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 3
        return self.damage-distance*0.25-0.0625*self.insects_shot if (self.damage-distance*0.25-0.0625*self.insects_shot)>0 else 0
        # END Problem Optional 3
