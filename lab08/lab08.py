...
def non_decrease_subseqs(s):
    ...
    def subseq_helper(s, prev):
        if not s:
            ...
            a=[] if len(s)==1 else s[1:]
            return insert_into_all(prev,subseq_helper(a,s[0]))+subseq_helper(a,s[0])
...
def merge(incr_a, incr_b):
    ...
    while next_b is not None or next_a is not None:
        if next_a is None:
            yield next_b
            yield from iter_b
            break
...
class Account:
    ...
    def __repr__(self):
        return f"Accountholder: {self.holder}, Deposits: {sum([item[0]=='deposit' for item in self.transactions])}, Withdraws: {sum([item[0]=='withdraw' for item in self.transactions])}"
...
def prune_small(t, n):
    ...
    while len(t.branches)>n:
        largest = max([item for item in t.branches], key=lambda x:x.label)
...
def long_paths(t, n):
    ...
    if n<=0 and t.is_leaf():
        ...
        ret=[]
        for item in t.branches:
            ret.extend([[t.label]+alpha for alpha in long_paths(item,n-1)])
        return ret
