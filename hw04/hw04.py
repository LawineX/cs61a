...
def replace_thor_at_leaf(t, thors_replacement):
    ...
    return tree(label(t),[replace_thor_at_leaf(i,thors_replacement) for i in branches(t)])
...
def has_path(t, word):
    ...
    if label(t)==word[0]:
        if len(word)==1:
            ...
            word=word[1:]
            return True in [has_path(i,word) for i in branches(t)]
    return False
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    assert upper_bound(interval(1 / upper_bound(y), 1 / lower_bound(y)))>lower_bound(interval(1 / upper_bound(y), 1 / lower_bound(y)))
